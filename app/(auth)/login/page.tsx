...
export default function LoginPage() {
  ...
  useEffect(() => {
    const handleAuth = async () => {
      ...
      if (token && stytchToken) {
        try {
          const result = await handleStytchCallback(stytchToken, orgId || undefined)
          if (result.success) {
  ...
  const handlePasswordLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.error_message || 'Authentication failed. Please check your credentials.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        <Card>
          ...
          <CardContent>
            <form onSubmit={isPasswordMode ? handlePasswordLogin : handleMagicLinkLogin}>
