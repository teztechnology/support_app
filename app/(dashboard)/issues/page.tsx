...
async function getPageData(organizationId: string) {
  try {
    const [issues, customers, categories, applications, users] = await Promise.all([
...
export default async function IssuesPage() {
  ...
  const { issues, customers, categories, applications, users } = await getPageData(session.organizationId)

  return <IssuesClient initialIssues={issues} customers={customers} categories={categories} applications={applications} users={users} />
}