...
export async function searchStytchMembers(emailQuery: string): Promise<ServerActionResponse<any[]>> {
  try {
...
export async function getOrganizationMembersWithStatus(): Promise<ServerActionResponse<{ stytchMembers: any[], databaseUsers: User[] }>> {
  try {
    const session = await SessionManager.requirePermission('settings:read')
...
export async function addUserToDatabase(stytchMemberId: string, role: string = 'support_agent'): Promise<ServerActionResponse<User>> {
  try {
    const session = await SessionManager.requirePermission('settings:write')
    
    // Get the Stytch member details
    const stytchMembers = await SessionManager.getOrganizationMembers(session.stytchOrganizationId!)
    const stytchMember = stytchMembers.find(m => m.member_id === stytchMemberId)
    ...
    const existingUser = await dbQueries.getUserByStytchMemberId(stytchMemberId)
    if (existingUser) {
      return {
        success: false,
    ...
    const createdUser = await dbQueries.createItem<User>('users', {
      ...newUser,
...
export async function updateUserRole(userId: string, role: string, permissions: string[]): Promise<ServerActionResponse<User>> {
  try {
    const session = await SessionManager.requirePermission('settings:write')
    
    const existingUser = await dbQueries.getUserById(userId, session.organizationId)
    if (!existingUser) {
      return {
        success: false,
...
export async function toggleUserActivation(userId: string): Promise<ServerActionResponse<User>> {
  try {
    const session = await SessionManager.requirePermission('settings:write')
...
export async function removeUserFromDatabase(userId: string): Promise<ServerActionResponse<void>> {
  try {
    const session = await SessionManager.requirePermission('settings:write')
