...
export class CosmosDBQueries {
  ...
  async createItem<T>(containerId: string, item: T, partitionKey: string): Promise<T> {
    try {
  ...
  async updateItem<T>(
    ...
    try {
      ...
      const { resource } = await container.item(id, partitionKey).replace(item as any, options)
      return resource as T
    } catch (error) {
  ...
  async getIssues(organizationId: string, filters: FilterParams = {}): Promise<Issue[]> {
    let query = `SELECT * FROM c WHERE c.organizationId = @organizationId`
    const parameters: any[] = [{ name: '@organizationId', value: organizationId }]
  ...
  async getIssueById(id: string, organizationId: string): Promise<Issue | null> {
  ...
  async updateIssue(
    ...
    return this.updateItem<Issue>('issues', id, updatedIssue, organizationId, etag)
  }

  async getCustomers(organizationId: string): Promise<Customer[]> {
    const querySpec: SqlQuerySpec = {
      query: 'SELECT * FROM c WHERE c.organizationId = @organizationId ORDER BY c.name ASC',
  ...
  async getCustomerById(id: string, organizationId: string): Promise<Customer | null> {
    return this.getItem<Customer>('customers', id, organizationId)
  }

  async createCustomer(customer: Omit<Customer, 'id' | 'createdAt' | 'updatedAt' | 'totalIssues'>): Promise<Customer> {
    ...
    return this.createItem<Customer>('customers', newCustomer, customer.organizationId)
  }
  ...
  async getOrganizationByStytchId(stytchOrganizationId: string): Promise<Organization | null> {
    const querySpec: SqlQuerySpec = {
      query: 'SELECT * FROM c WHERE c.stytchOrganizationId = @stytchOrganizationId',
      parameters: [{ name: '@stytchOrganizationId', value: stytchOrganizationId }]
    }

    const orgs = await this.queryItems<Organization>('organizations', querySpec)
  ...
  async getDashboardStats(organizationId: string, dateRange?: { start: string; end: string }): Promise<DashboardStats> {
    ...
    if (dateRange) {
      issueQuery += ' AND c.createdAt >= @startDate AND c.createdAt <= @endDate'
    ...
    const issuesByStatus: Record<IssueStatus, number> = {
      ...
      awaiting_customer: issues.filter(i => i.status === 'awaiting_customer').length,
    ...
    const activityQuery: SqlQuerySpec = {
      query: 'SELECT TOP 10 * FROM c WHERE c.organizationId = @organizationId ORDER BY c.timestamp DESC',
      parameters: [{ name: '@organizationId', value: organizationId }]
    }
    const recentActivity = await this.queryItems<ActivityItem>('activities', activityQuery)
