...
export function getStytchB2BServerClient(): StytchB2BClient {
  if (!stytchClient) {
    const projectId = process.env.STYTCH_BUSINESS_PROJECT_ID || process.env.STYTCH_PROJECT_ID
    const secret = process.env.STYTCH_BUSINESS_SECRET || process.env.STYTCH_SECRET
    ...
    try {
      ...
      console.warn('Stytch package not installed. Using mock client. Install with: npm install stytch')
      return createMockClient()
    }
  }
...
export async function authenticateB2BSession(sessionToken: string, organizationId?: string) {
  try {
